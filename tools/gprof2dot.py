...
class Main:
    ...
    def write_graph(self):
        ...
        if self.options.root:
            ...
            if not rootId:
                sys.stderr.write(
                    'root node ' + self.options.root + ' not found (might already be pruned : try -e0 -n0 flags)\n')
                sys.exit(1)
        ...
        if self.options.leaf:
            ...
            if not leafId:
                sys.stderr.write(
                    'leaf node ' + self.options.leaf + ' not found (maybe already pruned : try -e0 -n0 flags)\n')
                sys.exit(1)
